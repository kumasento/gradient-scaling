...
def get_train_dir(dtype=None,
    ...
    train_dir = '{rootdir}/{dataset}/{model}_{dtype}'.format(dataset=dataset,
                                                             model=model,
                                                             dtype=dtype,
                                                             rootdir=rootdir)
    if seed is not None:
...
def download_from_cluster(dtype,
                          dataset,
                          model,
                          job_id,
                          rootdir,
...
def train_on_cluster(train_script,
                     train_args,
                     cli_args=None,
...
def train_cifar_on_cluster(arch,
                           dataset,
                           dtype,
                           n_epoch,
                           schedule,
    ...
    label = '{arch}-{dataset}-{dtype}-{seed}'.format(arch=arch,
                                                     dataset=dataset,
                                                     dtype=dtype,
                                                     seed=manual_seed)
    ...
    if dtype == 'float32' and arch == 'resnet1202':
        cli_args.extend([
            '-A',
            'nvidia.k8s.pfn.io/gpu_model=Tesla-P100-PCIE-16GB',
        ])

    return train_on_cluster(train_script,
                            train_args,
                            label=label,
                            spec_file=spec_file)


def train_imagenet_on_cluster(arch,
                              dataset='imagenet',
                              dtype='float32',
                              manual_seed=0,
    ...
    spec_file = os.path.join(PROJDIR, 'examples', 'imagenet',
                             DEFAULT_SPEC_FILE)
    label = '{arch}-{dataset}-{dtype}'.format(arch=arch,
                                              dataset=dataset,
                                              dtype=dtype)
...
def load_train_log(dtype=None,
    ...
    if train_dir is None:
        ...
        train_dir = get_train_dir(dtype=dtype,
                                  dataset=dataset,
                                  model=model,
                                  rootdir=rootdir,
                                  seed=seed,
    ...
    if not os.path.isdir(train_dir):
        print('[WARN] train_dir to load data cannot be found: {}'.format(
            train_dir))
        return None
...
def plot_train_log(dataset=None,
                   model=None,
                   seed=None,
                   rootdir=None,
                   savedir=None,
    ...
    for dtype in dtypes:
        train_log = load_train_log(dtype,
                                   dataset,
                                   model,
                                   rootdir,
                                   seed=seed,
    ...
    if savedir is not None:
        ...
        fig.savefig(
            os.path.join(savedir, '{}_{}_train_log.pdf'.format(model,
                                                               dataset)))


def plot_train_logs(dataset,
                    models,
                    rootdir=None,
                    savedir=None,
                    x_axis='epoch',
    ...
    if savedir is not None:
        fig.savefig(
            os.path.join(
                savedir, '{}_{}_train_log.pdf'.format('_'.join(models),
                                                      dataset)))


def load_snapshot_and_create_model(arch,
                                   snapshot_name=None,
                                   snapshot_dir=None,
                                   dataset='imagenet',
                                   dtype='float32',
...
def get_snapshot_parameters(arch,
                            dtype,
                            snapshot_name=None,
                            dataset='imagenet',
                            rootdir=None,
    ...
    net = load_snapshot_and_create_model(arch,
                                         snapshot_name,
                                         dataset=dataset,
                                         dtype=dtype,
                                         rootdir=rootdir,
