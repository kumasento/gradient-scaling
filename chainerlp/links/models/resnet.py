...
class ResBottleneckBlock(PickableSequentialChain):
    ...
    def __init__(self,
        ...
        with self.init_scope():
            ...
            for i in range(n_layer - 1):
                name = 'b{}'.format(i + 1)
                bottleneck = Bottleneck(out_channels,
                                        mid_channels,
                                        out_channels,
                                        stride=1,
                                        dilate=dilate,
...
class ResNetCIFAR(PickableSequentialChain):
    ...
    def __init__(self,
        ...
        if initialW is None:
            initialW = self.get_initialW(use_fixup=use_fixup,
                                         L=sum(self._blocks[n_layer]),
                                         m=2)
        ...
        with self.init_scope():
            ...
            if use_fixup:
                self.bias1 = chainer.Parameter(initializer=initializers.Zero(),
                                               shape=1)
                self.bias2 = chainer.Parameter(initializer=initializers.Zero(),
                                               shape=1)
            ...
            self.res2 = ResBasicBlock(blocks[0],
                                      ...
                                      16,
                                      stride=1,
                                      res_scale=res_scales[0],
                                      **kwargs)
            self.res3 = ResBasicBlock(blocks[1],
                                      16,
                                      32,
                                      stride=2,
                                      res_scale=res_scales[1],
                                      **kwargs)
            self.res4 = ResBasicBlock(blocks[2],
                                      32,
                                      64,
                                      stride=2,
                                      res_scale=res_scales[2],
                                      **kwargs)
    ...
    def get_initialW(self, use_fixup=False, L=None, m=None):
        if not use_fixup:
            return initializers.HeNormal(scale=1., fan_option='fan_out')
        else:
